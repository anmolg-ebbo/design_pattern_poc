...
class EncryptedField:
    """Decorator to mark fields that need encryption"""
    def __init__(self, level: EncryptionLevel = EncryptionLevel.STANDARD):
        self.level = level

    def __call__(self, field: Field):
        field.encrypt = True
        field.encryption_level = self.level
        return field
...
class MongoDBConnection:
    ...
    def _setup_encryption(self):
        ...
        try:
            ...
            self.key_vault_client = MongoClient('mongodb+srv://agangwar0009:rDRC7JFeDYswSlSc@cluster0.hl9mt.mongodb.net/design_pattern_poc?retryWrites=true&w=majority&appName=Cluster0')
    ...
    def _build_schema_map(self, model: Type[BaseModel]) -> Dict:
        """Build MongoDB schema map from Pydantic model"""
        try:
            schema_map = { "bsonType": "object", "properties": {} }
            
            for field_name, field in model.__fields__.items():
                if hasattr(field, 'encrypt') and field.encrypt:
                    encryption_level = getattr(field, 'encryption_level', EncryptionLevel.STANDARD)
        ...
        try:
            ...
            with self._lock:
                if client_key not in self._client_pool:
                    ...
                    self._client_pool[client_key] = AsyncIOMotorClient(
                        config.MONGO_URI,
